...
mxMondrianBase.prototype.isDarkColor = function(color,colorIntensity)
{
	if(color === '#000000') // black
		...
		return false;
	else if(color === mxMondrianBase.prototype.CONFIG.COLOR_PALETTE.yellow.medium) // yellow medium is an exception. This will be light as well.
		return false;
	else
...
mxMondrianBase.prototype.getShapeVisualDefinition = function (
	...
	if(mxMondrianBase.prototype.graph['mondrianHighlightPredefinedEnabled'])
	{
...
mxMondrianBase.prototype.init = function(container)
{
	...
	mxMondrianBase.prototype.graph = this.state.view.graph;
...
mxMondrianBase.prototype.updateStyle = function(thisState, mandatoryStyles, defaultStyles, doBeginUpdate = true) {
	...
	{ 
		...
		if(mxMondrianBase.prototype.graph != undefined && mxMondrianBase.prototype.graph['mondrianHighlightPredefinedEnabled'])
		{
			let isPreDefined = ((thisState.cell.getAttribute('repoAttributes','')) != '');
			colorFamilyOverride = (isPreDefined) ? 'green' : 'red';
			colorIntensityOverride = (isPreDefined) ? mxMondrianBase.prototype.colorIntensity.MEDIUM : mxMondrianBase.prototype.colorIntensity.MEDIUM;
		}

		newStyles = mxUtils.setStyle(newStyles, 'strokeColor', 
			mxMondrianBase.prototype.getStrokeColor(newStyles, colorFamilyOverride, colorIntensityOverride));
	...
	if (this.changeListener == null)
	{
		this.changeListener = mxUtils.bind(this, function(sender, evt)
		{
			try
			{
				...
				else if(evt.properties.change.constructor.name === 'mxValueChange' && (evt.properties.change.cell.id === this.cellID))
				{
					let repoAttributes = mxMondrianBase.prototype.setAttributesFromRepo(this.state, 'Element-ID');
					mxMondrianBase.prototype.updateStyle(this.state, repoAttributes.repoFormatSettings, this.defaultStyleString);
...
mxMondrianBase.prototype.getStrokeColor = function (currentStyle, colorFamilyOverride, colorIntensityOverride) {
		let colorFamilyLine = (colorFamilyOverride) ?  colorFamilyOverride : mxMondrianBase.prototype.getStyleValue(
			currentStyle, 
			mxMondrianBaseConnector.prototype.cst.COLOR_FAMILY_LINE, 
			mxMondrianBaseConnector.prototype.cst.COLOR_FAMILY_LINE_DEFAULT);

		let colorIntensityLine = (colorIntensityOverride) ? colorIntensityOverride : mxMondrianBase.prototype.getStyleValue(
			currentStyle, 
		...
		return mxMondrianBase.prototype.getColor(
...
mxMondrianBaseConnector.prototype.setEdgeLabel = function(graph, cell, edgeChild)
{
	let isChild1 = false;
	let isChild2 = false;
	...
	if(cell.children != undefined)
	{
		for (let c = 0; c < cell.children.length; c++)
		{
			...
			let edgeChildNumber = mxMondrianBase.prototype.getStyleValue(child.style, mxMondrianBaseConnector.prototype.cst.EDGE_CHILD, undefined);
	...
	if( (edgeChild === mxMondrianBaseConnector.prototype.cst.EDGE_LABEL_1 && !isChild1) || (edgeChild === mxMondrianBaseConnector.prototype.cst.EDGE_LABEL_2 && !isChild2))
		mxMondrianBaseConnector.prototype.createEdgeLabel(graph, graph.view.getState(cell), edgeChild);
}
...
mxMondrianBaseConnector.prototype.addEdgeLabels = function(connector)
{
	if(connector.state != null && connector.state.cell != null && connector.state.view.getState(connector.state.cell) != null)
	{
		...
		if(cellState.cell.children != undefined)
		{
			...
			let el1Child = mxMondrianBase.prototype.getStyleValue(parent.style, mxMondrianBaseConnector.prototype.cst.EDGE_LABEL_1, undefined);
			let el2Child = mxMondrianBase.prototype.getStyleValue(parent.style, mxMondrianBaseConnector.prototype.cst.EDGE_LABEL_2, undefined);
			...
			for (let c = 0; c < cellState.cell.children.length; c++)
			{
				...
				{		
					...
					if(isChild1)
					{
						...
						parent.style = mxUtils.setStyle(parent.style, mxMondrianBaseConnector.prototype.cst.EDGE_LABEL_1, undefined);
					}
					else if(isChild2)
					{
						edgeChild = 2;
						child.style = mxUtils.setStyle(child.style, mxMondrianBaseConnector.prototype.cst.EDGE_CHILD, edgeChild);
						parent.style = mxUtils.setStyle(parent.style, mxMondrianBaseConnector.prototype.cst.EDGE_LABEL_2, undefined);
					}
				}
				else
				{
				...
				let edgeLabelAttribute = (isChild1) ? mxMondrianBaseConnector.prototype.cst.EDGE_LABEL_1_ATTRIBUTES : ((isChild2) ? mxMondrianBaseConnector.prototype.cst.EDGE_LABEL_2_ATTRIBUTES : undefined);
				let edgeLabelAttributes = (edgeLabelAttribute) ?  mxMondrianBase.prototype.getStyleValue(parent.style, edgeLabelAttribute, undefined) : undefined;

				if(edgeLabelAttributes != undefined && edgeLabelAttributes != 'undefined')
				{
					child.value.setAttribute('label', 
					mxMondrianBase.prototype.defineLabel(
						mxMondrianBaseConnector.prototype.getLabelFormat(edgeLabelAttributes), edgeLabelAttributes,
						child, 'defaultSettingsConnector'));
				}
			}
		}	
	}
...
mxMondrianBaseConnector.prototype.customProperties = [
	...
	{name: 'edgeLabel1Attributes', dispName: 'Label (Edge 1)', type: 'staticArr', subType: 'dynamicEnum', size: '3', subDefVal: 'noText',
		...
		onChange: function(graph, newValue)
		{
			...
			for (let i = 0; i < selectedCells.length; i++)
			{	
				mxMondrianBaseConnector.prototype.setEdgeLabel(graph, selectedCells[i], mxMondrianBaseConnector.prototype.cst.EDGE_LABEL_1);
			}
		}
	},
	{name: 'edgeLabel2Attributes', dispName: 'Label (Edge 2)', type: 'staticArr', subType: 'dynamicEnum', size: '3', subDefVal: 'noText',
		...
		onChange: function(graph, newValue)
		{
			...
			for (let i = 0; i < selectedCells.length; i++)
			{	
				mxMondrianBaseConnector.prototype.setEdgeLabel(graph, selectedCells[i], mxMondrianBaseConnector.prototype.cst.EDGE_LABEL_2);
			}
		}
	},
];
