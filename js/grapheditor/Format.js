...
DiagramFormatPanel.prototype.addMondrianOptions = function(div)
{
	...
	if (graph.isEnabled())
	{
		div.appendChild(this.createOption('Highlight Predefined', function()
		{
...
//			graph.updateCellStyles({'mondrianHighlightPredefined': (checked) ? '1' : null}, cells);
		},
		{
			install: function(apply)
			{
				this.listener = function()
				{
					apply(graph.mondrianHighlightPredefinedEnabled);
				};
				
				ui.addListener('mondrianHighlightPredefinedChanged', this.listener);
			},
			destroy: function()
			{
				ui.removeListener(this.listener);
