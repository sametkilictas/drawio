...
var EditDataDialog = function(ui, cell)
{
	...
	/** MONDRIAN - START - Adapt data dialog **/
	let shape = mxUtils.getValue(graph.getCurrentCellStyle(cell), 'shape');
	
	let mondrianShape = (shape === 'mxgraph.mondrian.base');
	let mondrianConnector = (shape === 'mxgraph.mondrian.connector');

	let addHeaderRow = function(index, position, value)
	{
		names[index] = null;
		let newRow = form.body.insertRow(position);
		newRow.innerHTML = `<th colspan=2 style='width: 100%; text-align: left; padding-left: 4px; padding-bottom: 4px; padding-top: 4px; background-color: #546E7A; color: white'>Mondrian Attributes: ${value}</td>`;
		texts[index] = newRow;
	};
	...
	if(mondrianShape || mondrianConnector)
	{
		if(mondrianShape)
		{
			$(document).ready(function() {
				var $select = $('#Icon-Name').selectize({
				maxItems: 1,
				options: window.MONDRIAN_REPO.ICONS,
				...
				searchField: ['name'],
				sortField: 'name',
				plugins: ['select_on_focus','clear_button']
				});
			});
		}
		...
		if(mondrianShape || mondrianConnector)
		{	
			let elementType = (mondrianShape) ? 'SHAPE' : 'INTERFACE';
			let client = 'default';
		
			let predefinedElements = [];
			let canvasCell = graph.getModel().getRoot();
			
			if(canvasCell != undefined && canvasCell.value != undefined)
			{
				predefinedElements = (canvasCell.hasAttribute('Predefined-Elements')) ? 
					(canvasCell.value.getAttribute('Predefined-Elements').split(',')).map( element => element.toLowerCase()) : [];

				client = predefinedElements[0]; // TODO: Support multiple clients, for now when can only use one
			}

			let availableOptions = window.MONDRIAN_REPO.getElementRepo(client, elementType);

			let optionID = (mondrianShape) ? value.getAttribute('Element-ID') : value.getAttribute('Interface-ID');
			let hasOptionID = (optionID != null);
			let isKnownPredefinedElement = (value.attributes.getNamedItem('repoAttributes') != null && value.attributes.getNamedItem('repoAttributes').value != "");			

			if(hasOptionID && !isKnownPredefinedElement)
			{
			...
			$(document).ready(function() {
				var $select = $((mondrianShape) ? '#Element-ID' : '#Interface-ID').selectize({
				maxItems: 1,
				...
				labelField: 'id',
				optgroupField: 'type',
				searchField: ['id', 'name', 'client', 'type', 'nameFull'],
				sortField: ['type', 'nameFull'], /*'id',*/
				plugins: ['select_on_focus', 'clear_button'],
				render: {
					option: function (item, escape) {
					  var label = item.id + ': ' + item.name; //item.id || item.name;
					  var caption = (item.client === 'default') ? item.nameFull : item.client + ': ' + item.nameFull;
					  return (
						"<div class='option'>" +
						escape(label) +
						(caption
						  ? "<span class='sub-option'>" + escape(caption) + "</span>"
						  : "") +
						"</div>"
					  );
					},
		...
		{
			if(attributesDefaultIndex[temp[i].name] != undefined)
			{
				mondrianAttributesDefault[attributesDefaultIndex[temp[i].name]] = temp[i];
			}
			else if(repoAttributes.includes(temp[i].name))
			{
				if(mondrianAttributesPredefined.length === 0)
			...
			else
			{
				if(mondrianAttributesCustom.length === 0)
					mondrianAttributesCustom.push({header: 'Custom'});
	...
	/** MONDRIAN - STOP - Adapt data dialog **/

	for (var i = 0; i < temp.length; i++)
	{
		if(temp[i].header != undefined)
		{
			addHeaderRow(count, count + 1, temp[i].header);
		}
		else
		{
			addTextArea(count, temp[i].name, temp[i].value);
			texts[count].setAttribute('id', temp[i].name);
		}
